		...
		for (i = 0; i < game.streams_count; ++i) {
			...
			if ((game.streams[i].buffer[0] & 0xFF) == ID_LOGIN) {
				...
				if (resource_pack_client_response.response_status == RESOURCE_PACK_CLIENT_RESPONSE_NONE || resource_pack_client_response.response_status == RESOURCE_PACK_CLIENT_RESPONSE_HAVE_ALL_PACKS) {
					...
					creative_content.size = 1;
					creative_content.entry_ids = (uint32_t *) malloc(sizeof(uint32_t));
					creative_content.entry_ids[0] = 0;
					creative_content.items = (misc_item_t *) malloc(sizeof(misc_item_t));
					misc_item_t item_test;
					item_test.network_id = 5;
					item_test.count = 64;
					item_test.metadata = 0;
					item_test.block_runtime_id = 5797;
					item_test.extra.with_nbt = 0;
					item_test.extra.can_place_on_size = 0;
					item_test.extra.can_destroy_size = 0;
					creative_content.items[0] = item_test;
